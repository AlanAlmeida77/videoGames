...
export const CLOSE_DETAIL_PAGE = 'CLOSE_DETAIL_PAGE'
...
export const getVideogames = (name) => {
  return async function (dispatch) {
    ...
    if (name) {
      try {
        const { data } = await apiRequest.get(`api/videogames?name=${name}`)
      ...
      try {
        const { data } = await apiRequest.get('api/videogames')
        const { ok, arrVideogames } = data
        if (ok) {
          dispatch({
            type: GET_VIDEOGAMES,
...
export const addVideogame = (newVideogame) => {
  return async function (dispatch) {
    try {
      const { data } = await apiRequest.post('api/videogames', newVideogame)
